...
export function AdminAnalyticsStats() {
  ...
  return (
    <section>
      ...
      {/* 核心指标 */}
      <div className={`grid grid-cols-2 lg:grid-cols-3 gap-px bg-border border border-border transition-opacity ${loading ? 'opacity-50' : ''}`}>
        {[
          ...
          { label: '实时在线', value: loading ? '…' : (data?.active ?? 0).toLocaleString(), sub: '5 分钟内活跃' },
        ].map(({ label, value, sub }) => (
