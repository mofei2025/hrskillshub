          ...
          {[
            ...
            <Link key={label} href={href}>
              <div className={`bg-card p-6 h-full hover:bg-[var(--hero-bg)] transition-colors ${urgent ? 'border-l-2 border-l-amber-400' : ''}`}>
                ...
                <p className={`font-heading text-4xl font-black ${urgent ? 'text-amber-500' : 'text-foreground'}`}>
                  {value}
                </p>
                {sub && (
                  <p className={`text-xs mt-2 font-mono ${urgent ? 'text-amber-500' : 'text-muted-foreground'}`}>
                    {sub}
                  </p>
                )}
              </div>
